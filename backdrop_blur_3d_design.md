...
**blur 是一个非局部（non-local）操作**——`blur(20px)` 意味着每个输出像素需要采样周围 20px 范围内的所有输入像素。直觉上，当父元素被切成两个片段分别做 backdrop blur 时，每片只采样本片区域会导致切割边界处产生接缝。**但实际上这个问题可以规避**：

**Chromium 的解决方式**（`skia_renderer.cc` `PrepareCanvasForRPDQ`）：

1. BSP 切割后每个碎片的 `draw_region` 不同，但 `visible_rect` 仍然是完整的原始 RPDQ bounds
2. `saveLayer` 使用完整的 `visible_rect` 作为 bounds 调用 backdrop filter（第 1636-1642 行），Skia 对完整 bounds 范围内的已绘制像素执行 blur
3. `ClearOutsideBackdropBounds` 通过 `clipPath(draw_region, kDifference)` + `clear(TRANSPARENT)` 清除 `draw_region` 之外的模糊内容（第 854-863 行）
4. 最终只保留当前碎片区域内的模糊结果

```
┌─────────────────┐
│  父元素远        │  ← 对完整 bounds 执行 blur，裁剪到"父元素远"区域
│  (模糊效果)      │
├── 切割边界 ──────┤  ← 无接缝：两次 blur 的输入范围相同，结果一致
│  父元素近        │  ← 对完整 bounds 执行 blur，裁剪到"父元素近"区域
│  (模糊效果)      │
└─────────────────┘
```

两个碎片各自独立做了**相同范围**的 blur，模糊结果一致，只是显示区域不同。拼接后自然连续，不存在接缝。

**TGFX 的实现同理**：模糊在完整的背景图（通过 `getBackgroundImage` 获取）上执行，生成完整的 resultImage，BSP 切割只影响最终 `drawPolygon` 的绘制区域。

**性能代价**：N 个碎片 = N 次完整范围的 blur 执行。碎片数量取决于场景中 3D 交叉的复杂度，blur 是 GPU 开销较大的操作，因此复杂场景下性能不可预测。

### 5.5 结论

| 场景 | backdrop-filter 是否可行 | 说明 |
|---|---|---|
| **子元素无交叉的简单情况** | ✅ 可行 | 父元素绘制时背后像素完整，不存在分片 |
| **子元素与父元素 3D 相交** | ✅ 技术可行，有性能代价 | 每个碎片对完整 bounds 做 blur 后裁剪，无接缝，但 blur 执行次数 = 碎片数 |
| **多子元素互相交叉** | ✅ 技术可行，性能代价更大 | 碎片更多，blur 执行次数更多 |

**Chrome 禁止同一元素 preserve-3d + backdrop-filter 的真正原因**不是技术障碍，而是多因素的综合决策：

1. **CSS 规范要求**：`backdrop-filter` 在 Chrome 中被实现为分组属性（`computed_style.h:2195` 中 `HasNonInitialBackdropFilter()` 检查），CSS Transforms Level 2 规范要求分组属性与 preserve-3d 共存时强制展平为 flat
2. **性能风险**：BSP 碎片数量不可预测，每个碎片独立执行完整 blur 的开销可能很高
3. **语义定义困难**：被 BSP 切割的元素，其子元素相对于切割碎片的层级关系难以在规范中精确定义
