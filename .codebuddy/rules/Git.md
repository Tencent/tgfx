...
完成任务后自动提交（仅 commit，**不自动推送**），无需用户额外指示。此规则仅适用于自动提交，**用户主动要求提交时，按用户指示决定提交范围**：

1. 若当前在 main 分支，先创建新分支
2. **提交本次会话中由你的操作引起的所有变更**（包括但不限于：代码编辑、文件删除/重命名、代码格式化、截图基准更新等），使用 `git commit --only <file1> <file2> ... -m "{Commit 信息}"`
