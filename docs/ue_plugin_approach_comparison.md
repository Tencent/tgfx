...
| UE Global Shader 模块 | .ush/.usf Shader + Permutation 预编译 | ~800 行 |
...
8. UE Global Shader 模块 (.ush/.usf + Permutation + 构建工具)          ~800 行
9. UERHICaps                                                          ~200 行
...
| 维度 | 方案 1 | 方案 2 |
|------|--------|--------|
| **Shader 编译时机** | tgfx 各后端运行时编译（shaderc/SPIRV-Cross） | **UE 构建时预编译**，运行时零编译开销 |
| **Shader 源码格式** | 与 tgfx 原生后端共享 GLSL | 需转换为 UE 的 .ush/.usf 格式（HLSL 兼容） |
| **特性组合** | tgfx 运行时动态拼装 GLSL | UE Shader Permutation 系统预编译所有组合 |
| **跨平台** | 每个平台各自编译（GLSL/MSL/HLSL） | UE 统一管线自动编译为各平台字节码 |

**方案 2 的 Shader 策略**（已被 Rive UE 插件验证）：将 tgfx 的 GLSL Shader 通过构建工具转换为
UE 兼容的 `.ush` 文件，使用 UE Global Shader 系统（`FGlobalShader`）注册，运行时特性组合通过
Shader Permutation（`SHADER_PERMUTATION_BOOL`）预编译覆盖，无需运行时 Shader 编译。
...
- **平台覆盖**：当前支持 D3D11 (Win64) 和 Metal (Mac/iOS)，架构上可扩展到所有 UE RHI 平台
