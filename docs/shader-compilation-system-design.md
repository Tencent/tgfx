...
一段 Shader 代码从源文本到在 GPU 上执行，必须依次经过三个不可跳过的阶段：
...
阶段 II: 驱动编译 (Driver Compilation)
  ...
  耗时: 1-10ms/shader (首次), <0.1ms (驱动缓存命中)
...
阶段 III: 管线状态组装 (Pipeline State Assembly)
  ...
  耗时: 1-5ms (首次), <0.1ms (PSO 缓存命中)
```

### 1.2 各阶段对首帧延迟的贡献

在未做任何优化的 TGFX 中，一个 Program（VS + FS）的首次创建耗时约 40-80ms，其中：

| 阶段 | 当前耗时 | 占比 | 可优化性 |
|------|:-------:|:----:|:--------:|
| I. 语言翻译 | 20-40ms | ~60% | 可完全离线化 |
| II. 驱动编译 | 3-10ms | ~20% | 可通过缓存序列化消除重复 |
| III. Pipeline 组装 | 1-5ms | ~10% | 可通过预创建消除首次延迟 |
...
阶段 I 的 Bundle 跳过了语言翻译，但 `vkCreateShaderModule` / `newLibraryWithSource` / `wgpuDeviceCreateShaderModule` 内部仍然需要 GPU 驱动将中间表示（SPIR-V / MSL / WGSL）编译为硬件微码。这一步耗时 1-10ms，且每次应用重启后都需要重复执行。
...
首次运行:
  创建空 VkPipelineCache
    → vkCreateGraphicsPipelines(pipelineCache, ...) — 每个 Pipeline 自动写入缓存
    → 帧结束或应用退出时:
       vkGetPipelineCacheData(pipelineCache, &size, &data)
       → 序列化到本地文件: pipeline_cache_vulkan.bin

后续运行:
  读取 pipeline_cache_vulkan.bin
    → vkCreatePipelineCache(createInfo.pInitialData = cachedData)
    → vkCreateGraphicsPipelines(pipelineCache, ...) — 驱动命中缓存, 编译接近瞬时
```

#### 3.3.2 Metal Binary Archive
...
首次运行:
  创建 MTLBinaryArchive
    ...
    → 帧结束或应用退出时:
       ...
       → 序列化到本地文件: pipeline_cache_metal.metallib

后续运行:
  读取 pipeline_cache_metal.metallib
    → [device newBinaryArchiveWithDescriptor:desc error:&error]
    → [device newRenderPipelineStateWithDescriptor:desc options:MTLPipelineOptionNone
...
| 指标 | 无缓存 (首次运行) | 有缓存 (后续运行) |
|------|:----------------:|:----------------:|
| 单 Pipeline 创建 | 3-10ms | **<0.5ms** |
| 首帧总开销 (阶段 I + II, 10 Pipelines) | 50-100ms | **5-10ms** |
...
UE5.1+ 引入了 PSO Precaching，基于组件加载时的材质和顶点工厂信息**自动预测**需要的 PSO：
...
应用启动
  ...
  └── PipelinePrecacher::start()                 [阶段 III 预创建]
        ...
        │     3. 组装 RenderPipelineDescriptor (使用常见渲染状态)
        │     4. createRenderPipeline → Program
        │     5. GlobalCache::addProgram(key, program)
        │
...
| 场景 | 首帧总开销 (10 Programs) |
|------|:----------------------:|
| 无任何优化 (当前状态) | **400-800ms** |
| +阶段 I: Bundle (跳过翻译) | **50-100ms** |
| +阶段 II: Pipeline Cache (跳过驱动重编译) | **5-10ms** |
| +阶段 III: Pipeline Precaching (跳过首次创建) | **<1ms** |
...
| **OpenGL** | ❌ 不需要 (驱动直接编译 GLSL) | ❌ 无标准 API | ⚠️ `glProgramBinary` 可选 |
