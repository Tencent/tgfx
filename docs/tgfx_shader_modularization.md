...
Phase 1-3 于 2026-04-07 完成，共 3 个 commit：
...
```
   ...
   (传统 GP emitCode 路径)         │                    (传统 XP emitCode 路径)
                    ...
                    ┌───────────────┼───────────────┐
                    │               │               │
              叶子 FP (模块化)  叶子 FP (fallback)  容器 FP (fallback)
              emitLeafFPCall()  emitCode()         emitCode()
              调用 .glsl 函数    传统字符串拼接       传统 emitChild()
```

**三类 FP 处理路径**：
1. **模块化叶子 FP**（13 个）：有对应的 `.glsl` 模块函数文件，通过 `emitLeafFPCall()` 生成函数调用 + uniform 声明
2. **Fallback 叶子 FP**（3 个）：TextureEffect、TiledTextureEffect、UnrolledBinaryGradientColorizer — 逻辑过于复杂（YUV 多通道、9 种 TileMode、8 层二叉查找），通过 legacy `emitCode()` 处理
3. **容器 FP**（4 个）：ClampedGradientEffect（已实现模块化内联展开）、ComposeFragmentProcessor、XfermodeFragmentProcessor、GaussianBlur1DFragmentProcessor — 后 3 个通过 legacy `emitCode()` 处理
...
| 文件 | 行数 | 说明 |
|------|------|------|
| `src/gpu/ModularProgramBuilder.h` | 92 | ModularProgramBuilder 类定义 |
| `src/gpu/ModularProgramBuilder.cpp` | 418 | 核心实现：FP 展开、模块函数调用、ClampedGradient 内联、fallback |
...
#### 决策 3：复杂叶子 FP 通过 fallback to emitCode() 处理

**选择**：TextureEffect/TiledTextureEffect/UnrolledBinaryGradientColorizer 不提取为 .glsl 函数

**理由**：
- TextureEffect：RGB/YUV 双路径 + 1-3 个 sampler + subset/alphaStart 变体 + 7 种色彩空间矩阵
- TiledTextureEffect：9 种 ShaderMode 的复杂坐标变换逻辑
- UnrolledBinaryGradientColorizer：基于 intervalCount 的二叉查找树 + 条件性 uniform 注册

这 3 个 FP 的 emitCode() 代码量合计约 800 行，且高度依赖运行时条件分支来决定 uniform 声明和代码结构，无法用简单的 `#define` + 函数签名表达。保留 emitCode() fallback 是零风险的选择。

#### 决策 4：容器 FP 通过 emitCode() fallback 处理（除 ClampedGradientEffect）

**选择**：ComposeFragmentProcessor、XfermodeFragmentProcessor、GaussianBlur1DFragmentProcessor 使用传统 `emitCode()` + `emitChild()` 递归路径

**理由**：
- 容器 FP 的 emitCode() 内部调用 `emitChild()` 递归展开子 FP，子 FP 可以是任意类型
- `emitChild()` 通过 `FragmentShaderBuilder` + `ProgramBuilder` 机制自动处理变量名 mangle 和 scope 隔离
...
| 项目 | 说明 | 优先级 |
|------|------|--------|
| 复杂叶子 FP 迁移 | TextureEffect/TiledTexture/UnrolledBinary 提取为 .glsl 模块 | 中 |
| 容器 FP 模块化展开 | Compose/Xfermode/GaussianBlur 替换 emitCode() fallback | 中 |
