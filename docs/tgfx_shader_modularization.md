...
### 17.22 Phase P 实施结果记录：GLSLBlend 死代码清理

> 2026-04-17 完成，共 1 个 commit。

#### 改动

Phase O 删除 FP/XP 的 `emitCode()` 后，`GLSLBlend.cpp` 中的 `AppendMode()` 和 `BlendModeName()` 只被 `XfermodeFragmentProcessor::emitContainerCode()` 调用。而 Xfermode 有 `shaderFunctionFile()` + `buildContainerCallStatement()`，始终走 `emitModularContainerFP()` 路径，`emitContainerCode()` 不可达。

| 文件 | 操作 | 删除行数 |
|------|------|---------|
| `src/gpu/glsl/GLSLBlend.cpp` | 整文件删除 | -551 |
| `src/gpu/glsl/GLSLBlend.h` | 整文件删除 | -31 |
| `XfermodeFragmentProcessor.cpp` | 删除 `emitContainerCode()` 实现 | -30 |
| `XfermodeFragmentProcessor.h` | 删除 `emitContainerCode()` 声明 | -5 |
| `GLSLPorterDuffXferProcessor.cpp` | 移除无用 `#include "GLSLBlend.h"` | -1 |
| `XfermodeFragmentProcessor.cpp` | 移除无用 `#include "GLSLBlend.h"` | -1 |
| **合计** | | **-620** |
...
| Commit | 说明 |
|--------|------|
| `64c85896` | 删除 GLSLBlend.cpp/.h + Xfermode emitContainerCode |
...
### 17.27 Phase T 实施结果记录：FragmentShaderBuilder legacy 回调清理
...
| 文件 | 操作 | 删除行数 |
|------|------|---------|
| `src/gpu/FragmentShaderBuilder.h` | 删除 `onBeforeChildProcEmitCode()` / `onAfterChildProcEmitCode()` 声明 + `class FragmentProcessor` 前向声明 | -5 |
| `src/gpu/FragmentShaderBuilder.cpp` | 删除两个方法的实现 | -8 |
| `src/gpu/processors/FragmentProcessor.h` | 删除无用 `#include <functional>` | -1 |
| **合计** | | **-14** |

`onBeforeChildProcEmitCode()` / `onAfterChildProcEmitCode()` 是 legacy `internalEmitChild()` 递归机制的回调钩子，用于在子 FP emit 前后 push/pop processor stack 做变量名 mangling。Phase S 删除 `internalEmitChild()` 后这两个方法已无调用者。

`<functional>` 此前用于 `emitChild()` 的 `std::function<std::string(std::string_view)> coordFunc` 参数，已随 Phase S 一并删除。

#### 完成的 Commit

| Commit | 说明 |
|--------|------|
| `91731b66` | 删除 FragmentShaderBuilder legacy 回调 + 无用 include |
...
Phase I–T（本轮 + 上轮会话）共完成 12 个清理阶段，总计删除约 **4200 行**代码。

#### 已消除的 legacy 机制

| 机制 | 消除阶段 |
|------|---------|
| 嵌入字符串双源（ShaderModuleRegistry.cpp 手工维护） | Phase I |
| `TGFX_USE_MODULAR_SHADERS` 编译开关 | Phase J |
| GP OFF 路径死代码 | Phase K |
| XP/GP legacy fallback | Phase L |
| FP `emitCode()` 调用依赖 | Phase M |
| ProgramBuilder 框架层死代码 | Phase N |
| 22 个 FP/XP `emitCode()` 实现 | Phase O |
| GLSLBlend.cpp/h + Xfermode `emitContainerCode` | Phase P |
| 3 个容器 FP 死 `emitContainerCode` + ProcessorGuard | Phase Q |
| ComposeFragmentProcessor 模块化迁移 + `emitContainerCode` 机制消除 | Phase R |
| FP/XP `emitCode` 纯虚消除 + EmitArgs/BuilderInputProvider/emitChild 基础设施 | Phase S |
| XferProcessor EmitArgs 清理 | Phase S 补充 |
| FragmentShaderBuilder legacy 回调 | Phase T |

#### 当前架构状态

- **FP/XP**：全部走 ModularProgramBuilder 模块化路径，无任何 legacy emit 残留
- **GP**：仍走 `emitCode(EmitArgs&)` 路径（活代码，非清理目标）
- **容器 FP**：5 个全部走 `emitModularContainerFP()`，`emitContainerCode` 机制已完全删除
- **Shader 模块**：.glsl 文件通过 CMake `embed_shaders.cmake` 自动生成 .inc，单一源头