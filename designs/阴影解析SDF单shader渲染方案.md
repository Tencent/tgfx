...
- **圆角矩形**：缩放后正圆角变为椭圆角（半轴 `r/σx`、`r/σy`），内层闭式积分用 3.1 的椭圆弧半宽公式。σx = σy 时两半轴相等、公式自动退化为正圆弧，因此**只有一条代码路径**，不需要分支判断各向同性与否。成本有两项：内层每次多一次乘法（`rx/ry` 比例因子），以及**求积点数在椭圆角悬殊时需从 4 提到 8**——归一化会放大形状的等效长宽比与角半轴差异，使求积误差上升（分档判据与实测见 5.2）。后者是主要成本。
...
- **参考解**：同一估计式、把外层求积点数取到很高的值当真值（该点数指单个探针点上外层求积的点数，与探针网格的点数是两回事，各处的取值就地注明；各向异性表另以原空间的暴力二维卷积为真值）。参考解本身也是数值近似，故用作真值前先自检一次——把求积点数翻倍，结果的变化应远小于被测误差：31×31 探针下 400 → 800 最大为 0.017 灰度级（出现在最极端的 half=(10, 2.5)、corner=(10, 0.25) 上，其余形状在 0.001~0.003 量级），而被测的 N=4 / N=8 误差在 0.5~16 灰度级，相差两个数量级以上，故参考解自身的残差不影响结论；
...
**求积点数按形状分档，取 4 或 8**。等向（σx = σy）下若干代表性形状（正圆、尖角矩形、极端长宽比等）的实测最大误差（15×15 铺满式探针，参考解 3000 点）为
...
加密到 21×21、参考解 4000 点后，k=2 那一行为 2.06 / 1.07 / 0.68 / 0.36，与 15×15 逐列相差不超过 0.02——探针加密只让最差点被更精确地捕捉到，量级不变。

读表结论：k 从 2 增到 3 时同一个 N 的误差仅小幅上升（N=4：2.05 → 2.26），k=4 起才明显退化（4.68）；同一 k 下 N 增大一半约使误差减半（4→6：2.05 → 1.05；8→12：0.66 → 0.34），N 加倍则降到约三分之一（4→8：2.05 → 0.66）；最差点集中在极端长宽比形状上——`halfX/halfY = 10/4` 与 `4/10` 取到 2.06，正圆仅 0.48。

**但 k × N 表只覆盖等向情形，各向异性下误差显著更高**。归一化按 `(1/σx, 1/σy)` 逐轴进行，形状的等效长宽比会被 σ 比例放大（例如 40×25 的形状在 σx=16 / σy=4 下变为 2.5 × 6.25，长宽比由 1.6 放大到 2.5），而极端长宽比正是求积误差的最坏场景。以原空间的暴力二维卷积为真值、9×9 铺满式探针实测：
...
因此单一取值面临两难：等向下 N=4 已够用（最差 2.06），但各向异性最差达 3.55；而统一取 8 则让所有形状都付 `2N + 2 = 18` 次 `Φ̃`，其中大多数形状用 4 就够。**故求积点数按形状分档，取 4 或 8。**

**判据只需一个量**，在 σ 归一化空间（即 shader 实际工作的空间）计算：

```
q = max(rx / ry, ry / rx)        两个归一化角半轴的比值

q > 2 时取 N = 8，否则取 N = 4
```

`q` 同时吸收各向异性 σ 与原形状自带的椭圆角：它大意味着角弧在某一轴方向被压扁，`rowSpan` 沿积分轴变化剧烈，外层求积误差随之抬高。

**半尺寸不进判据**。角半轴固定时，半尺寸对误差没有影响——此处用角部固定间距式探针，corner 取 (0.5, 0.5) / (1, 1) / (2, 1) / (2, 0.5)，半尺寸从 4 变到 32，各行误差分别恒为 1.24 / 1.55 / 2.30 / 4.79，一个数字都不变。只有当角半轴逼近半尺寸、且沿积分轴的半尺寸本身很小时，误差才随半尺寸变化（corner = (10, 0.5) 时，半尺寸从 (10,10) 缩到 (10,4)，误差由 11.26 涨到 16.28），成因是沿该轴的半尺寸越小、积分窗口越容易由形状截断，端点落在最外行的平方根发散点上。但这类形状的两个角半轴必然悬殊，`q` 已将其全部归入 N=8 一档，无需单列判据；反过来，角半轴相等的完全圆角形状（正圆、椭圆，`rx / halfX = 1`）误差仅 0.46~0.70，圆角占比本身并不指示风险。

**分档的通过判据**：落在 N=4 一档的形状，其误差不超过"统一取 8"在全空间已有的最差值——即分档不使总体变坏。在 371 个归一化形状（7 组半尺寸 × 角半轴两维各 9 档，涵盖长宽比、圆角占比、角半轴比值；31×31 铺满式探针，参考解 400 点中点求积）上实测：

| 方案 | 走 N=4 的形状占比 | N=4 一档最差 | 全空间最差 |
|---|---|---|---|
| 统一 N=8 | 0% | — | 7.90 |
| 统一 N=4 | 100% | 16.00 | 16.00 |
| `q > 1.5` | 25.6% | 4.98 | 7.90 |
| **`q > 2`（本方案）** | 43.9% | 6.09 | 7.90 |
| `q > 2.5` | 45.0% | 8.15 | 8.15 |
| `q > 3` | 55.8% | 8.15 | 8.15 |
| `min(rx, ry) < 1`（改用角半轴绝对值） | 52.6% | 15.57 | 16.00 |

本方案取 `q > 2`：N=4 一档最差 6.09，低于统一取 8 的 7.90，同时 43.9% 的形状把 `Φ̃` 调用从 18 次降到 10 次。阈值不能放宽——`q > 2.5` 一档中出现 8.15，已超过 7.90。改用角半轴的绝对值作判据则完全不成立（最差 15.57，与不分档几乎一样），因为误差由两个半轴的比例关系决定，单看其中较小的一个无法识别。

合成空间里 43.9% 这个占比偏低是因为它按参数均匀取样；实际内容中普通圆角卡片配等向 σ 时 `q = 1`，不触发加点，占比会显著更高。取 8 的是各向异性 σ 拉出的扁椭圆角，以及原形状自带的悬殊椭圆角。

被判据识别出的形状均具备"完全圆角 + 半轴悬殊"特征，最差三例：

| half | corner | q | N=4 | N=8 |
|---|---|---|---|---|
| (10.0, 4.0) | (10.00, 0.50) | 20.00 | 16.00 | 7.68 |
| (10.0, 4.0) | (10.00, 1.00) | 10.00 | 15.57 | 6.30 |
| (10.0, 2.5) | (10.00, 1.50) | 6.67 | 14.33 | 5.57 |

全空间 371 个形状中有 53 个在 N=4 下超过 7.90，全部落在判据的 N=8 一档内。

全空间 371 个形状中有 53 个在 N=4 下超过 7.90，全部落在判据的 N=8 一档内。

上表与本节的 k × N 表、各向异性场景表的形状集合、探针密度均不同（后两者为特定形状的逐例测量，本表为形状空间上的最差值搜索），**不可跨表比较绝对值**；表内比较才有效。探针密度的影响方向是单边的：铺满式网格的点数固定，密度不足时最差点被跳过，测出的值只会偏低不会偏高——同一形状用 15×15 测得 14.05，加密到 31×31 为 15.57、61×61 为 15.72、121×121 为 15.78。故各表数值均应视为该口径下的下界。
...
本方案不采用"1 个灰度级"作为硬性容差线——那是**单个孤立像素**上可察觉差异的经验下限，而此处的偏差是空间上缓变的低频量，可察觉阈值显著更高。真正的判据是最终截图上无可见 banding 与轮廓偏移。
...
**本方案不做 dither**。Figma 的四个闭式 shader 末尾均有 `dither(...)`（3.2）。不跟随的理由：TGFX 的 `src/` 与 `include/` 中当前不存在任何 dither 机制，渐变（`GLSLClampedGradientEffect`）等同样输出长距离平缓过渡的路径都未做，若只给阴影加，会出现"阴影有噪声抖动、相邻渐变没有"的不一致。

需要区分两类 banding 以免误判：**量化 banding** 源于 8-bit 输出本身（长距离平缓渐变每个色阶跨越多个像素），与 erf 精度、求积点数都无关，加点或换 erf 都消不掉，只能靠 dither 或更高位深；**近似误差导致的条纹**才与 N / erf 有关。因此实测若见 banding，先判断它是否随 N 增大而减弱——不减弱即为量化 banding，届时再评估是否为整个 TGFX 引入统一的 dither 机制（属独立改进）。

综合两项误差源，快路径的总偏差为求积的 6.09（N=4 一档的最差，N=8 一档为 7.90）与 erf 逼近的 0.124 个灰度级，量级由前者主导。与回落路径的差异则主要来自后者的离散化欠采样（4），其在小 σ 下可达 8.8~51 个灰度级，比快路径自身的近似误差大一个数量级。
...
| `RectInnerShadowFragmentProcessor` | `RectangleInnerShadow` | `(1 − shapeBlurRectCoverage)` × mask |
| `RRectInnerShadowFragmentProcessor` | `RoundedRectangleInnerShadow` | `(1 − shapeBlurRRectCoverage)` × mask |
...
- AA 沿用现状（2.4）：形状 mask 的覆盖率用 `shapeMaskCoverage`，与 `EllipseGeometryProcessor` / `RRectEffect` 一致的线性过渡 `clamp(0.5 − sdf, 0.0, 1.0)`，不引入 Figma 的 `smoothstep` 三次过渡（故不复用 3.2 的 `aaCoverage` 命名，避免与其 `fwidth` + `smoothstep` 实现混淆）。Inner 系的 mask 固定走这条软边，不设 Figma `useAAAMask` 那样的开关；
...
2. DropShadow 的 offset 是整体平移（由 `canvas->translate` 承担），但 InnerShadow 的 offset **只平移 shadow 形状、mask 不动**——平移 canvas 会把 mask 一起带走，故两套坐标之间存在相对偏移。

Inner 系的做法是挂两个 `CoordTransform`，由 FP 按索引取用（`transformedCoords[0]` 为 `maskCoord`、`[1]` 为 `shadowCoord`）。两个矩阵共用同一段前缀，只在后面各接一步：

```
maskCoordMatrix   = Trans(−maskCenter)                     // 调用方传入 uvMatrix 时 preConcat 在前
shadowCoordMatrix = maskCoordMatrix
                    后 postTranslate(maskCenter − shadowCenter)
                    后 postScale(1/σx, 1/σy)
```

即 `maskCoord` 停在 content 像素空间、相对 mask 形状中心，`shadowCoord` 再平移到 shadow 形状中心并逐轴除 σ。InnerShadow 的 offset 不进这两个矩阵，而是在 CPU 端直接加到 shadow 形状的 rect 上（平移 canvas 会把 mask 一起带走），故它经由 `shadowCenter` 体现在上式的平移项里。
...
**形状 mask 的 SDF 与覆盖率**（仅 Inner 系用，在 content 像素空间求值）。Inner 系的形状裁剪边不经过模糊，是一条硬切边，需要单独抗锯齿；该边同样由解析 SDF 给出，用 1 px 线性斜坡换算覆盖率即可，不必回到光栅化 mask，故单 pass、零中间纹理的结构不受影响。Blur 系没有这条边——其唯一可见边缘是模糊衰减边，本身平滑，不做也不需要抗锯齿。
...
`shapeMaskRRectSDF` 的角半轴取 `vec2` 而非标量，与 5.6 允许椭圆角进入快路径一致；标量版本无法表达椭圆角，会把 oval 的 mask 边算错。实现见 `src/gpu/glsl/processors/GLSLShapeBlurFunctions.cpp`。

`shapeMaskCoverage` 的 1 px 过渡带假设 SDF 已在 content 像素空间、且该空间到设备像素为等比且尺度接近 1；5.6 已把含透视或非等比缩放的形状排除到回落路径。

四类的 uniform 与主体如下。`coord` / `maskCoord` / `shadowCoord` 由 `CoordTransform` 产出，`shadowColor` 已预乘。四者末行统一乘上输入色的 alpha（`outColor *= inputColor.a`，承接 `paint.setAlpha`），下列主体省略该行。
...
**RectInnerShadowFragmentProcessor**（矩形内阴影）：uniform 为 `ShadowHalfOverSigma`（vec2）/ `MaskHalfSize`（vec2）/ `Color`：

```glsl
float coverage = 1.0 - shapeBlurRectCoverage(shadowCoord, shadowHalfOverSigma);
coverage *= shapeMaskCoverage(shapeMaskRectSDF(maskCoord, maskHalfSize));
outColor = shadowColor * coverage;
```

**RRectInnerShadowFragmentProcessor**（圆角内阴影）：uniform 为 `ShadowHalfOverSigma`（vec2）/ `ShadowCornerOverSigma`（vec2）/ `MaskHalfSize`（vec2）/ `MaskCornerRadius`（vec2）/ `Color`：

```glsl
float coverage = 1.0 - shapeBlurRRectCoverage(shadowCoord, shadowHalfOverSigma,
                                              shadowCornerOverSigma);
coverage *= shapeMaskCoverage(shapeMaskRRectSDF(maskCoord, maskHalfSize, maskCornerRadius));
outColor = shadowColor * coverage;
```

内部实心区跳过位于 `shapeBlurRRectCoverage` 内部，故 Inner 系调用同一函数时一并生效：在 shadow 形状的深处它返回 1，取反后 `1 − 1 = 0`，正是该处应有的结果。

`MaskHalfSize` / `MaskCornerRadius` 与 `maskCoord` 均在 content 像素空间（不除 σ），`ShadowHalfOverSigma` / `ShadowCornerOverSigma` 与 `shadowCoord` 已归一化——两组量所处空间不同，由两个 `CoordTransform` 各自的矩阵分开承担。
...
`InnerShadowStyle::onDraw`：同理按形状分派——`isRect` → `RectInnerShadowFragmentProcessor`，`isRRect` / 正圆 → `RRectInnerShadowFragmentProcessor`——传图层原形状（mask）+ spread 内缩形状（shadow）两组参数，offset 加在 shadow 形状上。
...
| shadow 形状 | `MakeSpreadRRect(rRect, ±spread)` 折算后 × contentScale；Inner 系再按 `(_offsetX, _offsetY) × contentScale` 平移（offset 只动 shadow 形状） |
...
| `MaskHalfSize` / `MaskCornerRadius`（仅 Inner） | mask 形状的半尺寸与角半轴，保持 content 像素空间、不除 σ |
...
| 脚本 | 结论 |
|------|------|
| `quadrature_study.py` | 等向情形下外层求积点数 N 的选型（核固定 ±2σ，21×21 探针）：N=4 最大偏差 2.06、N=8 为 0.68 个灰度级，最差点在长短边比 2.5:1 的形状上 |
...
| `anisotropic_reduction_study.py` | 各向异性归约的正确性验证：以原空间暴力二维卷积为真值，确认"按 (1/σx, 1/σy) 缩放 + 椭圆角"无系统偏差（N=32 时 ≤ 0.51 灰度级）；并测出各向异性下 N=4 最差 3.55、N=8 最差 1.49 |
...
| `quadrature_dispatch_rescan.py` | 分档判据的定稿依据：①固定间距探针下半尺寸对误差无影响（角半轴固定时半尺寸 4→32 误差不变），仅角半轴逼近半尺寸时才变化；②(rx, ry) 两维误差面；③371 个形状上的判据对照（31×31 探针，参考解 400 点），确认 `q > 2` 下 N=4 一档最差 6.09、不超过统一取 8 的 7.90，而 `q > 2.5` 起失效 |
