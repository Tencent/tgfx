...
**推广复杂度**：半宽公式按椭圆弧给出，故原形状自带椭圆角与"正圆角 + 各向异性 σ"落在同一公式上，均不需额外推广。仍需额外工作的是四角半径各异的情形：
...
**椭圆角随各向异性一并获得支持**。标量 σ 的归一化是等比缩放，正圆角缩放后仍是正圆角，故 Figma 的 `rowSpan` 用标量角半径与纯勾股半宽 `√(r²−d²)` 即精确（3.2）；而本方案的 σ 为 `vec2`，归一化是非等比缩放，正圆角**必然**变为椭圆角，`rowSpan` 因此必须按椭圆双半轴求值。两者是同一处改动的两面，不是两项独立特性。副产品是原形状自带的椭圆角也落在同一公式上——`RRect::MakeOval` 产出的各角半径等于对应边长之半，非正方形时即椭圆角，无需额外分支（5.6）。反之若沿用标量角半径去近似椭圆角，半宽误差可达数个像素（40×20 的 oval 上最差 4.09 或 10.00，取决于标量取哪个半轴），形状明显失真，故 Figma 对椭圆角只能回落通用路径。
