...
1. **高斯可分离**：`Gσ(x,y) = gσx(x)·gσy(y)`（轴对齐，两轴各自独立），二维卷积可按 Fubini 定理拆成两个一维积分的嵌套。其中一维高斯概率密度为
   ...
   **归约到各向同性**：把坐标按 `(x/σx, y/σy)` 缩放后，核成为 σ = 1 的各向同性单位高斯，形状则被非等比拉伸。该变换是精确的（不是近似）：各向异性高斯卷积在此缩放坐标系下严格等价于各向同性卷积，因为缩放同时作用于核与形状，且雅可比行列式常量可被归一化吸收。代价是形状的正圆角在缩放后变为**椭圆角**（x 半轴 `r/σx`、y 半轴 `r/σy`）。
...
2. **一维高斯积分有闭式**：累积分布函数 `Φσ(u) = ∫₋∞ᵘ gσ(t)dt = ½[1 + erf(u/(σ√2))]`，区间质量 `∫ₐᵇ gσ = Φσ(b) − Φσ(a)`。被卷积的 `f` 有限支撑——形状外恒为 0、对卷积无贡献，积分区间天然有限。
   ...
   **shader 侧无内置 erf**：GLSL / WGSL 均无（SPIR-V `GLSL.std.450` 亦无）；仅 Metal 的 MSL 提供 `metal::erf`，其实现闭源不可知，跨后端不可依赖，故 erf 必须手写。
   ...
   |------|------|------|---------|------|--------------|-----------|
   | A&S 7.1.26 | 单式：`t = 1/(1+px)`，`1 − P(t)·exp(−x²)` | 6 | 1 exp + 1 除 + 5 乘加 | 无 | 1.4e-7（公布的误差上限 1.5e-7） | 0.000 |
   | A&S 7.1.27 | 单式：`1 − 1/(1+a₁x+a₂x²+a₃x³+a₄x⁴)⁴`，纯有理式 | 4 | 1 除 + 4 乘加 | 无 | 4.7e-4（公布的误差上限 5e-4） | 0.124 |
...
**核截断对闭式的影响**：上文的高斯密度为理想无限支撑核——密度在任何有限距离外都不为零，而工程计算里要求模糊只能影响到有限距离，因此工程计算无法直接使用它：必须把核截断到有限区间，通常取 ±kσ（k 常取 2~3），并按截断区间的质量重新归一化以保持权重总和为 1。现状滤镜即取 k = 2（见 2.1）；紧支撑核（如三角核）天然有限支撑，无此问题。截断后 `Φ` 相应替换为截断核的 CDF：
...
**圆角矩形（逐行可分离 ⇒ 一维闭式 + 常数点数值积分）**（限定四角统一半径——四角相同才使每行横截面关于中心对称、可用单个半宽描述）：圆角使 `f` 不再完全分离，但固定一行 `y`，横截面仍是对称区间 `[−w(y), +w(y)]`，半宽解析。角半径的两个半轴记为 `rx`、`ry`：原形状为正圆角 `r` 时缩放后成为 `rx = r/σx`、`ry = r/σy`，原形状自带椭圆角时则各自缩放（二者相等时公式自动退化为正圆弧）：
...
内层是区间 `[−w(qy), +w(qy)]` 上的高斯积分，有闭式解（CDF 差）：

```
∫_{−w(qy)}^{+w(qy)} g(px−qx) dqx = Φ̃(px + w(qy)) − Φ̃(px − w(qy)) ≡ rowSpan(px, qy)
```

`rowSpan(x, y)` 即第 y 行的水平模糊覆盖率，取值 0~1。x 方向的积分至此做完，代回后外层成为一元积分：

```
C(p) = ∫ rowSpan(px, qy) · g(py − qy) dqy
```

外层因 `w(qy)` 含根号、与 `g` 复合后无初等原函数，只能数值计算，分四步展开。

**一、换元**。令 `s = py − qy`（相对目标点的行偏移），则 `qy = py − s`、`dqy = −ds`，积分限 `qy = y₀` / `y₁` 分别对应 `s = py − y₀` / `py − y₁`。逐步代入：

```
C(p) = ∫_{qy=y₀}^{y₁}      rowSpan(px, qy)     · g(py − qy) dqy
     = ∫_{s=py−y₀}^{py−y₁} rowSpan(px, py − s) · g(s) · (−ds)      代入 qy、dqy 与积分限
     = ∫_{s=py−y₁}^{py−y₀} rowSpan(px, py − s) · g(s) ds           交换积分限，两个负号抵消
```

两个因子对 `s` 的用法就此分开：`g` 只依赖目标点与该行的距离，换元后直接以 `s` 为自变量；`rowSpan` 只依赖该行在形状上的位置（由它决定半宽 `w`），故仍需绝对行坐标 `py − s`。

**二、收窄积分区间**。`g` 在 `|s| > k` 外为 0，`f` 在形状纵向范围 `[y₀, y₁]` 外为 0，后者换元后成为 `[py − y₁, py − y₀]`。两者的交集即有效区间 `[start, end]`：

```
start = clamp(−k, py − y₁, py − y₀)
end   = clamp(+k, py − y₁, py − y₀)
C(p) = ∫_start^end rowSpan(px, py − s) · g(s) ds
```

**三、提出核质量因子**。记该区间的核质量 `M = ∫_start^end g(s) ds = Φ̃(end) − Φ̃(start)`——`g` 的原函数就是 `Φ̃`，这一项有闭式。作恒等变形：

```
C(p) = M · [ (1/M) · ∫_start^end rowSpan(px, py − s) · g(s) ds ]
```

方括号内是 `rowSpan` 在该区间上按 `g` 分布取的加权平均值，为 0~1 的纯比值。该变形把外层积分中已知的部分（核在区间内的总量）与未知的部分（`rowSpan` 的相对分布）分开，只让后者承担离散化误差。

**四、离散加权平均**。取 N 个中点 `sᵢ = start + (i + 0.5)·step`（i = 0..N−1，`step = (end − start)/N`），分子与分母用同一套求和近似：

```
(1/M) · ∫_start^end rowSpan(px, py − s) · g(s) ds
    = ∫_start^end rowSpan(px, py − s)·g(s) ds / ∫_start^end g(s) ds    把 M 还原为积分形式
    ≈ [step · Σᵢ rowSpan(px, py − sᵢ)·g(sᵢ)] / [step · Σᵢ g(sᵢ)]       分子分母各用一次中点求积
    = Σᵢ rowSpan(px, py − sᵢ)·g(sᵢ) / Σᵢ g(sᵢ)                         step 为公共因子，约去
```

全链只有中间一步是近似，前后两步均为恒等变形。`g` 的常数因子 `1/(√(2π)·[Φ(k) − Φ(−k)])` 同样是公共因子、一并约去，故权重可直接取裸核值 `wᵢ = exp(−sᵢ²/2)`（归约后 σ = 1），求值时无需写出归一化常数。与第三步的 `M` 合并即得：

```
C(p) ≈ [ Σᵢ wᵢ · rowSpan(px, py − sᵢ) ] · (Φ̃(end) − Φ̃(start)) / Σᵢ wᵢ
```

**分母为何取离散权重和而非 M**：若分子离散、分母直接代入精确的 `M`，`M` 会与括号外的乘法约去，整式退回朴素中点求积 `Σᵢ rowSpan(px, py − sᵢ)·g(sᵢ)·step`，第三步的变形没有收益。分子分母同用一套求和，离散估算核总量的误差才会在相除时抵消，再乘回精确的 `M` 恢复正确尺度；等价于"朴素中点求积 × 真实核质量 / 离散估出的核质量"。收益在 `rowSpan` 沿 y 恒定时最直观：矩形各行半宽相同、圆角矩形内部实心区各行覆盖率恒为 1，此时加权平均精确等于该常数，结果与 N 无关；而朴素中点求积在 k = 2、N = 4 时把核质量估高 0.91%，这些点的覆盖率随之被整体放大约 2 个灰度级，与内部实心区跳过分支给出的精确 1.0 不一致。

两个轴的误差类型不同：**x 轴（内层）**为闭式 CDF 差，无采样误差，仅有所用求值方法带来的计算误差；**y 轴（外层）**为 N 点采样加权和，存在采样误差。

**外层采样误差由三个量决定**。先推出中点求积的误差上限。取宽为 `h` 的一段，中点记为 `m = a + h/2`，用中点处的函数值乘以宽度代替这一段的积分，误差有精确表达：

```
∫_a^{a+h} f ds − h·f(m) = (h³/24)·f″(ξ)
```

这个式子分三步得到：

1. 在中点 `m` 处把 `f` 按泰勒公式展开到二阶：`f(s) = f(m) + f′(m)·(s−m) + ½·f″(η)·(s−m)²`，其中 `η` 随 `s` 变化、落在 `m` 与 `s` 之间。
2. 两边在 `[m − h/2, m + h/2]` 上积分。常数项给出 `h·f(m)`，正是中点法的取值；一次项 `∫(s−m) ds` 因区间关于 `m` 对称而为 0——这一项消失正是中点法比取端点值精度高一阶的原因。
3. 只剩二次项 `½·∫ f″(η)·(s−m)² ds`。权重 `(s−m)²` 恒不为负，故可把 `f″` 提到积分外：连续函数按非负权重取加权平均，其值必等于该函数在区间内某一点 `ξ` 的取值，而 `ξ` 的位置未知。代入 `∫(s−m)² ds = h³/12`，得 `½·f″(ξ)·h³/12 = (h³/24)·f″(ξ)`。

第 3 步要求 `f″` 在该段上连续，这一前提在下文会再用到。

`ξ` 的位置未知，`f″(ξ)` 因此算不出来。把它换成整段上的最大值 `max|f″|`，等号随之变为不等号，得到单段的误差上限 `h³·max|f″| / 24`——这一步"用最大值替换未知值"正是结果只能是上限、而非精确值的原因。

再把整个区间 `[start, end]` 分成 N 段、每段宽 `h = (区间长度)/N`，逐段相加：

```
误差上限 = N · h³ · max|f″| / 24 = (区间长度)³ / (24 N²) · max|f″|

其中 f(s) = rowSpan(px, py − s) · g(s)
```

三个因子各自对应一个可控量：

1. **N（采样点数）**：误差上限按 `1/N²` 下降。
2. **区间长度**：归一化后窗口是核支撑 `±k` 与形状纵向范围的交集，长度不超过 `2k`；窗口越宽，同样的 N 摊得越薄。
3. **max|f″|（被积函数的曲率）**：由归一化后的两个角半轴 `rx/σx`、`ry/σy` 决定——既看两者的比值（比值越大，弧在积分轴方向被压得越扁），也看它们各自的数值本身（归一化空间里曲率的量纲是 1/长度，半轴数值越小、弧越弯）。`rowSpan` 沿积分轴变化越剧烈，这一项越大。

**该上限只用于说明三个因子的作用，不用于算出具体数值**，原因有二。一是它的适用对象是朴素中点求积 `Σᵢ rowSpan(px, py − sᵢ)·g(sᵢ)·step`，即把整个被积函数离散求和；本方案的估计式只离散 `rowSpan` 的加权平均、核质量取闭式精确值 `Φ̃(end) − Φ̃(start)`，两式在同一套求积点下相差"闭式核质量 / 离散估出的核质量 `Σᵢ g(sᵢ)·step`"这一标量因子。该因子可精确算出，故这一条只要求套用前按它校正，并不阻断套用。二是决定性的一条：它要求被积函数在该段上二阶连续可导，而外层被积函数并不总满足——半宽 `w` 在形状最外行处按平方根变化（`√(rx² − δ²)` 在 `δ → rx` 时切线竖直），该点上 `w` 仍连续但已不可导，一阶导与二阶导在其邻域内均无界。积分段的端点一旦落在这一点上，`max|f″|` 即为无穷，上限失去意义。具体取值只能靠数值实测。

结论：N 与 k 必须一起选定——脱离 k 谈某个 N 够不够没有意义；而在 k 固定之后，N 所需的大小仍随归一化几何而变。

**推广复杂度**：仍需额外推广的只有四角半径各异（per-corner）的情形——x、y 两个方向都不能再用 `abs()` 折叠对称：横截面的左右边界要分别计算，上下两侧的圆角区也要分别计算。
...
**坐标归一化在顶点阶段完成**：四个闭式 shader 的 fragment 侧都不含"目标点减边界"的算式，该步骤由配套 vertex shader 承担并经 varying 插值传入。`(p − 边界) / blurRadius` 对顶点位置是仿射函数，光栅化的重心插值对仿射量精确，故插值结果与在 fragment 内逐像素现算完全一致。四份 vertex 源码（同样取自 WASM，uniform 块与各自 fragment 逐字对应）：

```glsl
// RectangleBlur vertex：quad 按 blurRadius 外扩得顶点位置 p，
// 再打包四边偏移 (px−x₀, py−y₀, px−x₁, py−y₁) / blurRadius
void main() {
  vec2 p = mix(quad.xy - blurRadius, quad.zw + blurRadius, position2);
  normEdges = (vec4(p, p) - quad) / blurRadius;
  gl_Position = vec4(vertexTransform * vec3(p, 1), 0).xywz;
}

// RoundedRectangleBlur vertex：圆角路径需要"相对中心"的坐标，故减中心而非减四边
void main() {
  vec2 center = (quad.xy + quad.zw) * 0.5;
  vec2 p = mix(quad.xy - blurRadius, quad.zw + blurRadius, position2);
  coord = (p - center) / blurRadius;
  gl_Position = vec4(vertexTransform * vec3(p, 1), 0).xywz;
}

// RectangleInnerShadow vertex：quad 按 maskBounds 外扩，但偏移减的是 shadowBounds
//（两个 bounds 之差即承载 offset）；canvasPos 未归一化，供 mask SDF 使用
void main() {
  vec2 p = mix(maskBounds.xy - blurRadius, maskBounds.zw + blurRadius, position2);
  normShadowEdges = (vec4(p, p) - shadowBounds) / blurRadius;
  canvasPos = p;
  gl_Position = vec4(vertexTransform * vec3(p, 1), 0).xywz;
}

// RoundedRectangleInnerShadow vertex：两套坐标各减自己形状的中心，
// 阴影侧归一化、mask 侧保持像素单位；quad 取 maskBounds 且不外扩
void main() {
  vec2 maskCenter = (maskBounds.xy + maskBounds.zw) * 0.5;
  vec2 shadowCenter = (shadowBounds.xy + shadowBounds.zw) * 0.5;
  vec2 p = mix(maskBounds.xy, maskBounds.zw, position2);
  centeredNormShadowCoords = (p - shadowCenter) / blurRadius;
  centeredMaskCoords = p - maskCenter;
  gl_Position = vec4(vertexTransform * vec3(p, 1), 0).xywz;
}
```

由此可读出三点。**一是矩形闭式解的完整对应**：把 `normEdges` 展开代入下文方式 1 的 `axisAlignedCoverage`，即得 3.1 的矩形纯闭式解 `[Φ̃(px−x₀) − Φ̃(px−x₁)] · [Φ̃(py−y₀) − Φ̃(py−y₁)]`，两式逐项一致。**二是 Inner 系用两个 varying 承载两套坐标**：阴影穿透区在归一化空间求值，形状 mask 在像素空间求值（其 AA 过渡带为 1 px、与 blurRadius 无关），且 InnerShadow 的 offset 只平移阴影形状、mask 不动，故两套坐标间存在相对偏移。**三是 Inner 两个变体的 quad 外扩不同**：矩形变体按 `maskBounds ± blurRadius` 外扩，圆角变体取 `maskBounds` 不外扩——内阴影不会画到形状之外，圆角变体的取法才是紧致的；矩形变体的外扩部分被 `1 − coverage` 与 mask 裁掉，属冗余而非错误。

**绘制区域的范围扩展**：模糊会把影响范围推出形状边界——三角核紧支撑 [−1, +1]（归一化单位），物理上即形状 bounds 向外扩展 **1 × blurRadius**：宽高 100、blurRadius = 10 的矩形，阴影绘制区域（quad）为 120 × 120（再叠加 offset 平移）。归一化式 `coord = (fragPos − center) / blurRadius` 的除数即支撑半径，故核的外缘恰好落在 quad 边界上。上列 vertex 源码中 Drop 系的 `mix(quad.xy - blurRadius, quad.zw + blurRadius, position2)` 即该扩展的实现，`position2` 为单位方格上的顶点参数。
...
**输出后处理：dither**。四个闭式 shader 的末行都把着色结果交给同一个 `dither`，它给每个像素叠加一个幅度约 `±1/255` 的伪随机偏移，把 8-bit 量化的色阶边界打散。随机源取 `gl_FragCoord.xy`，即该像素的屏幕坐标（由光栅化器产出，每个像素唯一）：

```glsl
const float DITHER_AMPLITUDE = 0.007843;   // 约 2/255

// ditherNoise：以屏幕坐标为输入的伪随机哈希，取值 [0, 1)
float ditherNoise() {
  vec2 fragPos = gl_FragCoord.xy;
  return fract(sin(dot(vec2(12.9898, 78.233), fragPos)) * 43758.5453);
}

// dither：alpha 为 0 的像素原样返回，避免在完全透明处引入噪声
vec4 dither(vec4 color) {
  float offset = (ditherNoise() - 0.5) * DITHER_AMPLITUDE;
  return color.a > 0.0 ? min(abs(color + offset), 1.0) : color;
}
```

两个细节：偏移量按 `(noise − 0.5)` 取中心对称，故不改变平均亮度；`color.a > 0.0` 的判断让完全透明的区域（阴影影响范围之外）保持纯透明，不被噪声污染。本方案不跟随该机制，理由见 5.2。
...
InnerShadow 主体引用的图层形状 SDF 与抗锯齿覆盖，在提取源码中的定义如下。`roundRectSDF` 由轴对齐矩形 SDF 减去角半径做圆角化，角半径 clamp 到不超过半尺寸；`perCornerRectSDF` 支持四角各自不同的半径，但每角仍是标量圆形角，传全零半径时展开即与 `boxSDF` 逐项相同；`aaCoverage` 把 SDF 按 `fwidth` 归一化到像素单位，在 ±0.5 px 过渡带内做 `smoothstep`：
...
// perCornerRectSDF：四角各一个标量半径的圆角矩形 SDF，入参为 bounds（非 halfSize）
//   p       相对 bounds 左上角的坐标
//   bounds  形状的左上 / 右下角
//   radii   四角半径，按象限选取其中一个后与 roundRectSDF 同式
float perCornerRectSDF(vec2 p, vec4 bounds, vec4 radii) {
  vec2 halfSize = (bounds.zw - bounds.xy) / 2.0;
  vec2 rel = p - halfSize;
  vec2 pair = rel.x > 0.0 ? radii.zy : radii.wx;
  float r = rel.y > 0.0 ? pair.x : pair.y;
  vec2 d = abs(rel) - halfSize + r;
...
| 椭圆角（rx ≠ ry） | 支持（光栅化真实轮廓） | 未涉及 | ❌ 闭式不支持：`rowSpan` 的角半径为标量、半宽取纯勾股 `√(r²−d²)`，<br>椭圆角回落通用 tile 路径 |
...
本方案沿用 3.2 闭式路线的结构（内层闭式 + 外层固定点数求积、自适应选轴、内部实心区跳过、spread 在 CPU 端折算），**但核函数取 ±2σ 截断高斯而非 Figma 的三角核**：在阴影绘制末端，当 StyledShape 可解析为 **fill 类型的 Rect / RRect / Oval（四角统一圆角，可为椭圆角；无 corner smoothing）**时，构造对应 FragmentProcessor 单 pass 绘制阴影；stroke 类型及其余形状回落现状滤镜路径。
...
代价是核求值需 `exp` 与多项式（erf 逼近）而非纯分段二次多项式。该成本落在每次 `Φ̃` 调用上（矩形 4 次、圆角 `2N + 2` 即 10 或 18 次，N 按形状分档、见 5.2），仍是与 σ 无关的常数，不改变"单 pass、零中间纹理、常数采样"的量级结论。
...
- **圆角矩形**：缩放后正圆角变为椭圆角（半轴 `r/σx`、`r/σy`），内层闭式积分用 3.1 的椭圆弧半宽公式。σx = σy 时两半轴相等、公式自动退化为正圆弧，因此**只有一条代码路径**，不需要分支判断各向同性与否。成本有两项：内层每次多一次乘法（`rx/ry` 比例因子），以及**求积点数在椭圆角悬殊时需从 4 提到 8**——归一化会拉大两个角半轴的差异（q），使求积误差上升（分档判据与实测见 5.2）。后者是主要成本。
...
**椭圆角随各向异性一并获得支持**，两者是同一处改动的两面，不是两项独立特性。Figma 无此需要：其 σ 为标量、归一化是等比缩放，正圆角缩放后仍是正圆角，故 `rowSpan` 用标量角半径与纯勾股半宽 `√(r²−d²)` 即精确（3.2）。副产品是原形状自带的椭圆角也落在同一公式上——`RRect::MakeOval` 产出的各角半径等于对应边长之半，非正方形时即椭圆角，无需额外分支。反之若沿用标量角半径去近似椭圆角，半宽误差可达数个像素（40×20 的 oval 上最差 4.09 或 10.00，取决于标量取哪个半轴），形状明显失真，故 Figma 对椭圆角只能回落通用路径。
...
代价是优化覆盖不到 spread = 0 的场景，而这是阴影的多数用法。解除该限制的前提是让 `contentShape` 与contour 图像的覆盖范围一致，届时只需去掉这一条判定。
...
两个自由参数需定值：外层求积点数 N、erf 逼近式；并用实测数据复核 5.1 选定的核截断范围 k = 2 在精度上无代价。

**N 只能由实测定**。3.1 已说明那个误差上限 `(区间长度)³/(24N²)·max|f″|` 不能用来算具体数值：积分段的端点一旦落在形状最外行，`f″` 在趋近该点时发散，该段上的 `max|f″|` 取不到有限值。该上限只能说明 N、窗口长度、曲率三者的作用方向，取值由实测给出。

**测定术语**三项：

- **参考解**：同一估计式、把外层求积点数取到很高的值当真值（该点数指单个探针点上外层求积的点数，与探针网格的点数是两回事，各处的取值就地注明；归约精确性验证另以原空间的暴力二维卷积为真值）。参考解本身也是数值近似，故用作真值前先自检一次——把求积点数翻倍，结果的变化应远小于被测误差：41×41 探针、参考解 400 → 800 点在最差形状（half = (10, 2.5)、corner = (10, 0.5)）上最大漂移 0.010 灰度级，而被测的 N=4 / N=8 误差在 2~16 灰度级，相差两个数量级以上，故参考解自身的残差不影响结论；
- **探针网格**：两种布法，各处只注明用的是哪一种与密度。**铺满式**在形状及其模糊影响范围（`±(half + 2)`）内等分布点，点数固定，用于在同一形状内找最差点；**角部固定间距式**只覆盖一个圆角附近区域（`[halfX − rx − 2.2, halfX + 2.2] × [halfY − ry − 2.2, halfY + 2.2]`，其中 2 为核支撑半径）、间距固定 0.2，用于跨半尺寸比较——铺满式的点数固定，形状越大间距越大，会跨过误差集中的角弧带，不同半尺寸的数值不可比；
- **误差口径**：N 点结果与参考解之差的绝对值，在探针上取最大，乘 255 换算成 8-bit 灰度级。

**k = 2 的精度复核**。同一 N 下截断窗口越宽、同样的点数摊得越薄，误差应随 k 单调上升；实测如下（9 个代表形状，半尺寸 0.5~20，覆盖正圆、部分圆角与长宽比 20:1 的细长形状，15×15 铺满式探针，参考解 3000 点，取全形状最差）：

| k（σ） | N=4 | N=8 | N=16 |
|--------|-----|-----|------|
| 2.0（本方案所取，亦为现状滤镜的截断半径） | 2.05 | 0.66 | 0.22 |
| 3.0（Wallace shader 所用） | 2.26 | 0.81 | 0.32 |
| 4.0 | 4.68 | 1.74 | 0.63 |

三列下 k = 2 均为最低，5.1 的核选型在精度上无代价；k = 4 起明显退化。

**求积点数按形状分档，取 4 或 8**：归一化角半轴的比值 `q > 2` 时取 N = 8，否则取 N = 4。判据在 σ 归一化空间（即 shader 实际工作的空间）计算，输入只有归一化角半轴 `rx`、`ry` 两个量。分档的必要性来自归一化对角形状的拉伸：归一化把正圆角拉成椭圆角（半轴 `r/σx`、`r/σy`），σ 比例越悬殊、或原形状自带的椭圆角越偏心，两个归一化角半轴的差异越大——`q` 大意味着角弧在某一轴方向被压扁，`rowSpan` 沿积分轴变化剧烈，外层求积误差随之抬高。误差由角半轴的差异主导、而非形状的归一化长宽比（分离这两个因素的实验见 6.3 的 `aspect_vs_corner_study.py`）。归一化角半轴的比值记为

```
q = max(rx / ry, ry / rx)
```

误差随 `q` 增大而上升意味着统一 N 不可行，需在归一化形状空间做最差值搜索：6 组半尺寸（(10,10)、(6,6)、(4,4)、(10,4)、(10,2.5)、(4,2.5)，长宽比 1:1 到 4:1）× 角半轴两维各 9 档（0.25~10），共 337 个形状，41×41 铺满式探针，参考解 400 点。半尺寸不进判据：角半轴固定时，半尺寸从 4 变到 32 实测误差不变；唯一例外是角半轴逼近半尺寸的形状，此时两个角半轴必然悬殊。按 `q` 分两档统计各 N 的最差误差：

| 形状子集 | 形状数 | N=4 | N=8 | N=16 | N=32 |
|---|---|---|---|---|---|
| 全空间 | 337 | 16.01 | 7.90 | 3.81 | 1.58 |
| q ≤ 2 | 151 | 6.16 | 2.07 | 0.71 | 0.25 |
| q > 2 | 186 | 16.01 | 7.90 | 3.81 | 1.58 |

读表结论：全空间统一 N=4 最差 16.01，不可接受；统一 N=8 最差 7.90，但 `q ≤ 2` 的形状在 N=4 下已达 6.16、低于 7.90——统一取 8 让这批形状白付一倍的 `Φ̃` 调用（每次求值由 `2N + 2` 即 10 次升到 18 次），故按 `q` 分档取 4/8。N=16 / N=32 列显示继续加点仍有收益（3.81 / 1.58），但调用次数成倍增长，4/8 是收益与成本的拐点。阈值取 2 而非更松的 2.5 / 3：后两者的 N=4 一档最差为 7.57 / 7.82，距 7.90 的余量仅 4% / 1%，而探针网格测出的最差值是下限估计、加密只会上升（同一形状从 15×15 加密到 121×121 升幅约 12%），薄余量可能反转；阈值 2 的余量为 22%，放宽到 2.5 仅多放行 3 个形状。改用角半轴绝对值作判据（`min(rx, ry) < 1` 时取 N=8）则 N=4 一档最差 15.59，与不分档几乎一样——误差由两个半轴的比例关系决定，单看较小的一个无法识别。实际内容中普通圆角卡片配等向 σ 时 `q = 1`，走 N=4；取 8 的是各向异性 σ 或自带悬殊椭圆角的形状。

k 表与 N 表的形状集合与探针密度不同（前者 9 个代表形状 × 15×15，后者 337 个形状 × 41×41），数值不可跨表比较绝对值。

**代价**：`quadratureCount` 参与 shader 源码生成、进入 program key（`onComputeProcessorKey` 写入该值），故两个取值意味着 `RRectBlur` 与 `RRectInnerShadow` 各最多编译 2 个 program。该 keying 机制本就存在，不是分档新增的。CPU 侧多两行判断。

本方案不采用"1 个灰度级"作为硬性容差线——那是**单个孤立像素**上可察觉差异的经验下限，而此处的偏差是空间上缓变的低频量，可察觉阈值显著更高。真正的判据是最终截图上无可见 banding 与轮廓偏移（8-bit 量化固有的条带除外：长距离平缓渐变下即使近似误差为零，色阶条带也必然存在）。

**erf 逼近取 A&S 7.1.27 完整四系数版**（3.1 候选表第二行）。理由：该式为纯有理式、**不需 `exp`**，而圆角路径每像素调 `Φ̃` 达 `2N + 2` 即 10 或 18 次，`exp` 在 GPU 上是多周期特殊函数指令；其 4.7e-4 的精度传导到可观察量后仅 0.124 个灰度级，落在 8-bit 量化噪声以下，故不选精度高三个数量级但需 `exp` 的 7.1.26。保留 a₃ = 0.000972（Wallace 省去了它）：该项使误差从 8.2e-4 降到 4.7e-4、回到公布的 5e-4 上限内，代价仅一次乘加。该上限已在本仓库复验（实测 4.659e-4，见 6.3）。另一个优点是它对全体 x ≥ 0 有效，不与核截断范围 `k` 耦合。

**若 erf 精度成为瓶颈**（实测出现无法靠加点消除的 banding），升级序列为：先换 7.1.26（同接口同出处，只需替换 `shapeBlurErf` 一个函数，代价是每次调用一个 `exp`）；若 `exp` 开销不可接受，再考虑有界区间拟合多项式（精度更高且无 exp，但系数需自行拟合、与 `k` 隐式耦合）。

综合两项误差源，快路径的总偏差为求积的 6.16（N=4 一档的最差，N=8 一档为 7.90）与 erf 逼近的 0.124 个灰度级，量级由前者主导。与回落路径的差异则主要来自后者的离散化欠采样（4），其在小 σ 下可达 8.8~51 个灰度级，对等向典型形状而言比快路径自身的近似误差（0.68~2.06）大一个数量级。
...
**canvas 矩阵的处理**：`Layer::drawLayerStyleDefault` 在调用 `onDraw` 前给 canvas concat 了 `Scale(1/contentScale) · Translate(contentEntry.offset)`（`Layer.cpp:1946-1948`），而 `styleInput.contentOffset` 取的正是同一个 `contentEntry.offset`（`Layer.cpp:1951`）。记该偏移为 `off`、缩放为 `cs`，设 `C` 为图层 local → 设备的矩阵，则局部点 `p` 映射到设备为 `C · ((p + off)/cs)`，即 `p` 的图层 local 坐标为 `(p + off)/cs`，反解得 `p = cs × 图层local − off`。**因此 `onDraw` 里的 canvas local 空间就是 content 像素空间**，与本节选定的坐标系一致——Shader 的采样坐标与 drawRect 都不需要再做 contentScale 换算。

Shader 侧因此只需把 canvas local 坐标平移到形状中心、再逐轴除 σ，该矩阵由 `asFragmentProcessor` 交给 `CoordTransform`（调用方传入 `uvMatrix` 时 preConcat 在前）：

```
totalMatrix = Trans(−center) 后 postScale(1/σx, 1/σy)   // center 为形状中心，在 content 像素空间
```

**阴影绘制区域**（drawRect，content 像素空间）：

```
DropShadow：spreadBounds.makeOutset(2σx, 2σy)
            offset 由 canvas->translate(_offsetX × contentScale, _offsetY × contentScale) 承担
InnerShadow：图层原形状 bounds（阴影不外扩，与 filterBounds 返回 srcRect 一致）
            offset 施加在 shadow 形状上而非 canvas 上——平移 canvas 会把 mask 一起带走
```

其中 `spreadBounds` 为 spread 折算后的形状 bounds（`MakeSpreadRRect` 的结果）。`2σ` 即截断核支撑半径，故 drawRect 边界处覆盖率严格为 0、不产生被裁切的硬边。
...
**合批能力的变化需纳入性能评估**：现状路径的 `canvas->drawImage` 在 brush 相同时可合批，而阴影图像各不相同（每个图层的模糊结果是独立纹理），实际也难合批；新路径下同参数阴影反而**可以**合批（几何走 RectDrawOp 的顶点数据、参数走 uniform）。但**参数不同的阴影一律打断合批**，这是相对"pass 数减少"的一项反向成本，量级需实测确认。
...
| 处理器 | 对齐 Figma | 核心 |
|--------|-----------|------|
| `RectBlurFragmentProcessor` | `RectangleBlur` | `shapeBlurRectCoverage` 零采样 |
| `RRectBlurFragmentProcessor` | `RoundedRectangleBlur` | `shapeBlurRRectCoverage`，4 或 8 采样（按形状分档） |
| `RectInnerShadowFragmentProcessor` | `RectangleInnerShadow` | `(1 − shapeBlurRectCoverage)` × shadow 形状覆盖率 |
| `RRectInnerShadowFragmentProcessor` | `RoundedRectangleInnerShadow` | `(1 − shapeBlurRRectCoverage)` × shadow 形状覆盖率 |

四者本期都是**阴影专用**：均以 `shadowColor` 为 uniform、输出 `shadowColor × coverage`，不是可直接复用的通用模糊原语。前两个（Blur 系）的覆盖率计算部分（`shapeBlurRectCoverage` / `shapeBlurRRectCoverage` 及其函数链）与阴影语义无关，是后续扩展的复用点；把上色从 FP 中剥离本期不做，留待第一个真实复用场景落地时按需重构。
...
- 共享 GLSL 函数以固定名字用 `FragmentShaderBuilder::addFunction` 声明，由 `AppendRectBlurFunctions` / `AppendRRectBlurFunctions` / `AppendRectMaskFunctions` / `AppendRRectMaskFunctions` 四个入口发射（`src/gpu/glsl/processors/GLSLShapeBlurFunctions.h`）。不使用 `getMangledFunctionName` 生成唯一名，改为约定：每个入口在同一 builder 上最多调用一次，且同组的 Rect 与 RRect 变体互斥。这样同一 program 内不会出现同名不同体的函数，也不会因改名产生重复代码；
- AA 沿用现状（2.4）：形状 mask 的覆盖率用 `shapeMaskCoverage`，与 `EllipseGeometryProcessor` / `RRectEffect` 一致的线性过渡 `clamp(0.5 − sdf, 0.0, 1.0)`，不引入 Figma 的 `smoothstep` 三次过渡（故不复用 3.2 的 `aaCoverage` 命名，避免与其 `fwidth` + `smoothstep` 实现混淆）。Inner 系的 shadow 形状裁剪边固定走这条软边，不设 Figma `useAAAMask` 那样的开关；
...
- **图层 alpha 沿用 `paint.setAlpha`，不需并入 `shadowColor`**：`Paint::setAlpha` 写的是 `brush.color.alpha`（`Paint.h:85-87`），该 color 会随顶点记录进入 GP（`OpsCompositor.cpp` 的 `RectRecord(rect, matrix, brush.color)`），由 GP 写入 FP 链的输入色（`GLSLQuadPerEdgeAAGeometryProcessor.cpp:55-63` 的 `args.outputColor`）。FP 只取输入色的 alpha 相乘（`outColor *= inputColor.a`），颜色本身一律来自 `shadowColor` uniform——故 alpha 对 shader 输出生效，而输入色的 RGB 不会污染阴影色。已实测确认：白色 color shader 配 `setAlpha(0.5)`，输出为 128。因此现状 `paint.setAlpha(alpha)`（`DropShadowStyle.cpp:158`）的传递方式可原样保留；
- **输出不做 dither**：Figma 的四个闭式 shader 末尾均有 `dither(...)`（3.2），本方案不跟随：TGFX 的 `src/` 与 `include/` 中当前不存在任何 dither 机制，渐变（`GLSLClampedGradientEffect`）等同样输出长距离平缓过渡的路径都未做，若只给阴影加，会出现"阴影有噪声抖动、相邻渐变没有"的不一致。

**坐标传递**：Blur 系只需一套 σ 归一化坐标，由单个 `CoordTransform` 承担（其 uvMatrix 合并了中心平移与 `1/σ` 归一化，见 5.3.1）。Inner 系需要**两套坐标**，原因有二：

1. mask（穿透区）在 σ 归一化空间求值，而 shadow 形状的 AA 过渡带是 1 个 content 像素、与 σ 无关，必须在未归一化空间求值；
2. DropShadow 的 offset 是整体平移（由 `canvas->translate` 承担），但 InnerShadow 的 offset **只平移 mask 形状、shadow 形状不动**——平移 canvas 会把 shadow 形状一起带走，故两套坐标之间存在相对偏移。

Inner 系的做法是挂两个 `CoordTransform`，由 FP 按索引取用（`transformedCoords[0]` 为 `shadowCoord`、`[1]` 为 `maskCoord`）。两个矩阵共用同一段前缀，只在后面各接一步：

```
shadowCoordMatrix = Trans(−shadowCenter)                   // 调用方传入 uvMatrix 时 preConcat 在前
maskCoordMatrix   = shadowCoordMatrix
                    后 postTranslate(shadowCenter − maskCenter)
                    后 postScale(1/σx, 1/σy)
```

即 `shadowCoord` 停在 content 像素空间、相对 shadow 形状中心，`maskCoord` 再平移到 mask 形状中心并逐轴除 σ。InnerShadow 的 offset 不进这两个矩阵，而是在 CPU 端直接加到 mask 形状的 rect 上（平移 canvas 会把 shadow 形状一起带走），故它经由 `maskCenter` 体现在上式的平移项里。

**核函数链**：σ 归一化后两轴 σ 恒为 1、支撑恒为 ±2，`Φ̃` 的两个端点质量退化为编译期常量，故截断 CDF 只需一次 `shapeBlurErf`：

```glsl
// shapeBlurErf：erf 的逼近式，源自 Hastings (1955)，即 Abramowitz & Stegun
// "Handbook of Mathematical Functions" 式 7.1.27（p. 299）:
//   https://personal.math.ubc.ca/~cbm/aands/page_299.htm
// 适用域 0 <= x < inf，公布的误差上限 |e(x)| <= 5e-4。纯有理式，无需 exp。
...
// shapeBlurCDF：sigma 归一化后（sigma = 1）截断到 ±2 并重新归一化的核 CDF。
// 记 Phi 为未截断的 CDF，Phi(u) = 0.5 * (1 + erf(u / sqrt(2)))，则结果为
// (Phi(u) - Phi(-2)) / (Phi(2) - Phi(-2))：0.70710678 是 1/sqrt(2)，
// 0.02275013 是 Phi(-2)，0.95449974 是 Phi(2) - Phi(-2) 即窗口内保留的质量。
...
// shapeBlurRectCoverage：矩形的纯闭式覆盖率，两方向各一次 CDF 差后相乘，零采样。
...
float shapeBlurRectCoverage(vec2 coord, vec2 halfSize) {
  ...
  return (shapeBlurCDF(lo.x) - shapeBlurCDF(hi.x))
       * (shapeBlurCDF(lo.y) - shapeBlurCDF(hi.y));
}
```

`shapeBlurSliceCoverage` / `shapeBlurRRectCoverage` 的结构照 3.2 的 `rowSpan` / `roundedRectShadow` 移植，三处改动：`gaussianCDF` 换成 `shapeBlurCDF`、`triWeight(s)` 换成 `exp(-0.5 * s * s)`、核支撑常数 `1.0` 换成 `2.0`（含 `clamp(±1.0, lo, hi)` → `clamp(±2.0, lo, hi)`）。此外角半径由单一 `corner` 改为椭圆双半轴，以支持各向异性 σ（5.1）。

命名约定：**along 指内层闭式积分所沿的那个轴，cross 指与它垂直、由外层求积扫过的轴**。3.2 的 `rowSpan` 把"行"固定为水平方向，这里两轴要互换角色，故改用 along / cross 表述：

```glsl
// shapeBlurSliceCoverage：点在垂直于积分轴的某一条切片上，沿 along 轴的闭式模糊覆盖率。
//   alongCoord   该点在内层积分轴上的归一化坐标
//   crossCoord   该点在外层求积轴上的归一化坐标（决定处于直边区还是圆角区）
//   cornerAlong  内层积分轴方向的角半轴（决定半宽的收缩量）
//   cornerCross  外层求积轴方向的角半轴（决定直边区/圆角区分界）
//   halfAlong / halfCross  两轴的归一化半尺寸
// 调用方保证 cornerAlong <= halfAlong 且 cornerCross <= halfCross。
...
上式假定 `cornerAlong ≤ halfAlong` 且 `cornerCross ≤ halfCross`（否则 `halfAlong − cornerAlong` 为负、半宽算错）。该前提由 `RRect::setRectRadii` 的 `ScaleRadii` 保证：相邻两角半径之和超过所在边长时按比例整体缩小（`RRect.cpp:45-79`），故单角半径不超过对应半边长。而 `MakeSpreadRRect` 折算后同样调用 `setRectRadii`（`SpreadUtils.cpp:50-52`），因此 **spread 折算的结果也满足该不变式**——包括"角半径 + spread 后逼近半尺寸"的情形（如 100×60、r=50/30 叠加 spread=10 后正好 r=60/40 = halfSize，即退化为 oval，仍然合法）。

另需注意 `ScaleRadii` 会把任一分量为 0 的角整体归零（`RRect.cpp:74-78`），故"x 分量非零、y 分量为零"的畸形角不会到达 shader。

`shapeBlurRRectCoverage` 相应把 `corner` 参数改为 `vec2 corner`（两个半轴），在两条选轴分支里按积分轴分配：沿 y 求积时内层沿 x，取 `alongCoord = coord.x`、`crossCoord = coord.y − s`、`cornerAlong = corner.x`、`cornerCross = corner.y`；沿 x 求积时两轴交换。

内部实心区跳过的判据同样按 `vec2` 写、支撑常数取 2.0：点在两个方向上离边界的距离都超过"核支撑半径 2.0 加圆角弧的内推量"时，核完全不跨边界，覆盖率恒为 1，可跳过积分。该判据在椭圆角下已数值验证：在判据边界上取最坏点，真实覆盖率与 1.0 的偏差为 9.4e-9（纯数值积分残差，0.0000 灰度级），即判据充分保守。形状小于核时（如 `halfSize − corner < 2`）右侧为负、条件恒假，自动退回积分路径，无需额外保护。

**shadow 形状的 SDF 与覆盖率**（仅 Inner 系用，在 content 像素空间求值）。Inner 系的形状裁剪边不经过模糊，是一条硬切边，需要单独抗锯齿；该边同样由解析 SDF 给出，用 1 px 线性斜坡换算覆盖率即可，不必回到光栅化 mask，故单 pass、零中间纹理的结构不受影响。Blur 系没有这条边——其唯一可见边缘是模糊衰减边，本身平滑，不做也不需要抗锯齿。

```glsl
// shapeMaskRectSDF：轴对齐矩形 SDF，p 相对中心（外正内负）
float shapeMaskRectSDF(vec2 p, vec2 halfSize) {
  vec2 d = abs(p) - halfSize;
  return length(max(d, vec2(0.0))) + min(max(d.x, d.y), 0.0);
}

// shapeMaskRRectSDF：椭圆角圆角矩形的 SDF，角半轴为 vec2。
// 角区外由包围矩形给出精确距离；角区内 d 为到角椭圆中心的偏移、r 为其半轴，
// F = dot(d/r, d/r) − 1 在椭圆上为零，边界附近 F 约等于 length(grad F) 乘距离，
// 故取 F / length(grad F) 作为近似——椭圆的精确距离没有闭式。
// 半轴先 clamp 到不超过半尺寸（超出时退化为胶囊而非把内矩形翻转），
// 再夹到非零以保证除法有限。
float shapeMaskRRectSDF(vec2 p, vec2 halfSize, vec2 corner) { ... }

// shapeMaskCoverage：SDF 转覆盖率，1 px 线性过渡（对齐 TGFX 现状 AA，非 smoothstep）
float shapeMaskCoverage(float sdf) {
  return clamp(0.5 - sdf, 0.0, 1.0);
}
```

`shapeMaskRRectSDF` 的角半轴取 `vec2` 而非标量，与 5.6 允许椭圆角进入快路径一致；标量版本无法表达椭圆角，会把 oval 的形状裁剪边算错。实现见 `src/gpu/glsl/processors/GLSLShapeBlurFunctions.cpp`。

`shapeMaskCoverage` 的 1 px 过渡带假设 SDF 已在 content 像素空间、且该空间到设备像素为等比且尺度接近 1；5.6 已把含透视或非等比缩放的形状排除到回落路径。

四类的 uniform 与主体如下。`coord` / `shadowCoord` / `maskCoord` 由 `CoordTransform` 产出，`shadowColor` 已预乘。四者末行统一乘上输入色的 alpha（`outColor *= inputColor.a`，承接 `paint.setAlpha`），下列主体省略该行。

**RectBlurFragmentProcessor**（矩形通用模糊）：uniform 为 `HalfSizeOverSigma`（vec2）/ `Color`。零采样（对应 3.1 矩形纯闭式解）：

```glsl
float coverage = shapeBlurRectCoverage(coord, halfSizeOverSigma);
outColor = shadowColor * coverage;
```

**RRectBlurFragmentProcessor**（圆角通用模糊）：uniform 为 `HalfSizeOverSigma`（vec2）/ `CornerOverSigma`（vec2，椭圆双半轴）/ `Color`。内部实心区跳过与 4 / 8 点求积都在 `shapeBlurRRectCoverage` 内部完成（采样数由 CPU 按 5.2 的判据选定、以字面量写入 shader 源码并进 program key）：

```glsl
float coverage = shapeBlurRRectCoverage(coord, halfSizeOverSigma, cornerOverSigma);
outColor = shadowColor * coverage;
```

**RectInnerShadowFragmentProcessor**（矩形内阴影）：uniform 为 `MaskHalfOverSigma`（vec2）/ `ShadowHalfSize`（vec2）/ `Color`：

```glsl
float coverage = 1.0 - shapeBlurRectCoverage(maskCoord, maskHalfOverSigma);
coverage *= shapeMaskCoverage(shapeMaskRectSDF(shadowCoord, shadowHalfSize));
outColor = shadowColor * coverage;
```

**RRectInnerShadowFragmentProcessor**（圆角内阴影）：uniform 为 `MaskHalfOverSigma`（vec2）/ `MaskCornerOverSigma`（vec2）/ `ShadowHalfSize`（vec2）/ `ShadowCornerRadius`（vec2）/ `Color`：

```glsl
float coverage = 1.0 - shapeBlurRRectCoverage(maskCoord, maskHalfOverSigma,
                                              maskCornerOverSigma);
coverage *= shapeMaskCoverage(shapeMaskRRectSDF(shadowCoord, shadowHalfSize, shadowCornerRadius));
outColor = shadowColor * coverage;
```

内部实心区跳过位于 `shapeBlurRRectCoverage` 内部，故 Inner 系调用同一函数时一并生效：在 mask 形状的深处它返回 1，取反后 `1 − 1 = 0`，正是该处应有的结果。

`ShadowHalfSize` / `ShadowCornerRadius` 与 `shadowCoord` 均在 content 像素空间（不除 σ），`MaskHalfOverSigma` / `MaskCornerOverSigma` 与 `maskCoord` 已归一化——两组量所处空间不同，由两个 `CoordTransform` 各自的矩阵分开承担。

Inner 语义（双形状：`shadow*` = 图层原形状，即阴影填充与裁剪范围；`mask*` = spread 内缩并加 offset 后的形状，其模糊覆盖率被取反扣除）与现有 `InnerShadowStyle::drawWithSpread` 的滤镜语义一致（`InnerShadowStyle.cpp:143`，现状用 `MakeSpreadShapeImage(input, 0)` 取原形状、`MakeSpreadShapeImage(input, -_spread)` 取穿透形状）。注意命名与 3.2 提取的 Figma `RoundedRectangleInnerShadow` 源码相反——Figma 的 `maskBounds`/`shadowBounds` 分别指原形状与穿透形状；本方案按仓库滤镜路径的语义命名，对照 Figma 源码时需交换两个前缀理解。

### 5.5 DropShadowStyle / InnerShadowStyle 集成

`DropShadowStyle::onDraw`：StyledShape 可解析且 spread ≠ 0 时按形状分派——`isRect` → `RectBlurFragmentProcessor`，`isRRect` / 正圆 → `RRectBlurFragmentProcessor`——经 5.3 的 Shader 单 pass 绘制，旁路 `SpreadUtils::MakeSpreadShapeImage` + blur 滤镜；否则回落现有路径。spread → (bounds, corner) 复用 `MakeSpreadRRect` 的折算，避免两条路径几何不一致。offset 由 `canvas->translate` 承担（5.3.1）。
...
`InnerShadowStyle::onDraw`：同理按形状分派——`isRect` → `RectInnerShadowFragmentProcessor`，`isRRect` / 正圆 → `RRectInnerShadowFragmentProcessor`——传图层原形状（shadow）+ spread 内缩形状（mask）两组参数，offset 加在 mask 形状上。

**CPU 端需算好的量**（均在 content 像素空间，见 5.3.1）：

| 量 | 算法 |
|---|---|
| `σx` / `σy` | `_blurrinessX/Y × contentScale`；任一为 0 时不构造 blur FP |
| shadow 形状 | Drop 系：`MakeSpreadRRect(rRect, +spread)` 折算后 × contentScale；Inner 系：原 rRect × contentScale |
| mask 形状（仅 Inner） | `MakeSpreadRRect(rRect, −spread)` 折算后 × contentScale，再按 `(_offsetX, _offsetY) × contentScale` 平移（offset 只动 mask 形状） |
| `HalfSizeOverSigma`（Inner 系为 `MaskHalfOverSigma`） | 被模糊形状的 halfSize / vec2(σx, σy)（Drop 系为 shadow 形状，Inner 系为 mask 形状） |
| `CornerOverSigma`（Inner 系为 `MaskCornerOverSigma`） | `vec2(r / σx, r / σy)`（同一个被模糊形状的角半径 r，两轴各自归一化 ⇒ 椭圆双半轴） |
| `ShadowHalfSize` / `ShadowCornerRadius`（仅 Inner） | shadow 形状（原形状）的半尺寸与角半轴，保持 content 像素空间、不除 σ |
| 两个坐标矩阵（仅 Inner） | 见 5.4 坐标传递 |
| `quadratureCount`（仅 RRect 系） | 由归一化后的 `CornerOverSigma` 算出 `q = max(rx/ry, ry/rx)`，按 5.2 的判据取 4 或 8 |
| `drawRect` | 见 5.3.1 |
...
**banding 的归因**：审阅时若见到条带，先区分两类——**量化 banding** 源于 8-bit 输出本身（长距离平缓渐变每个色阶跨越多个像素，即使近似误差为零也存在），与 erf 精度、求积点数都无关，加点或换 erf 都消不掉；**近似误差导致的条纹**才与 N / erf 有关。判据是看它是否随 N 增大而减弱：不减弱即为量化 banding，不按回归处理。
...
| 各向异性 σ | 统一圆角 RRect，fill | Drop，spread = 1，σx = 12 / σy = 4 | 椭圆角的 `shapeBlurSliceCoverage` 路径（5.1）：角半径两个半轴不等，验证归一化正确、无方向性畸变 |
...
- **σ = 0（任一轴）**：核退化，`shapeBlurCDF` 的归一化除数与坐标矩阵里的 `1/σ` 缩放均失效，须在 CPU 端拦截——任一轴 σ 为 0 时不构造 blur FP，回落现状滤镜路径（现状对 blur = 0 已有 nearest 采样的专门处理，`DropShadowStyle.cpp:146-148`；且 `ImageFilter::Blur` 在两轴皆 0 时返回 nullptr）。**不自行按"硬边形状"绘制**：现状 spread ≠ 0 且 blur = 0 时阴影边缘保留 1 px 光栅化 AA（2.4 表末行），改走硬边会使这批场景外观回退。
- **负 spread 导致形状收缩消失**：CPU 端由 `MakeSpreadRRect` 折算出空 rect，不绘制（与 `IsSpreadCollapsed` 现有语义一致，`SpreadUtils.cpp:135`）。
- **角半径 = 半尺寸（正圆 / 椭圆极值）与角半径 = 0（尖角）**：`shapeBlurSliceCoverage` 的圆角区 / 直边区分界退化。尖角时两个角半轴同为 0，`ratio = cornerAlong / max(cornerCross, 1e-6)` 的兜底分母避免除零（分子同为 0，故 ratio = 0），`sqrt` 项被 `max(0, ...)` 归零，`curved` 正确退化为 `halfAlong`。角半径 = 半尺寸时 `halfAlong − cornerAlong` 为 0，`curved` 退化为 `(cornerAlong/cornerCross)·√(cornerCross² − delta²)`，即椭圆的半宽。断言两种极值下覆盖率无 NaN。
- **σ 远大于形状尺寸（形状被完全"吞没"）**：外层积分窗口被形状范围截断而非核截断，验证 `start == end` 的退化分支返回 0 且不产生 NaN。
- **求积点数分档的两条分支各自命中**：构造三组形状分别落在 `q > 2`、`q ≤ 2`、`q` 恰好等于 2 的边界上，断言 CPU 侧算出的点数与 5.2 判据一致；尖角（角半径为 0）无圆弧、`q` 无定义，断言归入 N=4 一档。
...
- contentScale ≠ 1（图层带缩放）：验证 5.3.1 的坐标空间约定成立——阴影的虚化范围应随缩放等比变化，且与回落路径一致。σ 与几何都按 contentScale 换算，而 canvas local 空间已是 content 像素空间，两者中任何一处多乘或少乘一次 contentScale 都会在此暴露。
...
`designs/shadow-sdf-scripts/` 下的 13 个脚本是全文数值结论的唯一来源，可随参数改动重跑复核。它们是方案论证的实验记录，不是工程代码：随设计文档归档，不参与构建，也无需按项目的代码规范维护。

各脚本承载的结论：

| 脚本 | 结论 |
|------|------|
| `quadrature_study.py` | 等向典型场景下外层求积点数 N 的余量（核固定 ±2σ，10 个代表形状，21×21 探针，参考解 4000 点）：N=4 / 8 / 16 / 32 的最大偏差 2.06 / 0.68 / 0.23 / 0.08 个灰度级，最差点在长短边比 2.5:1 的形状上 |
| `window_vs_samples_study.py` | 核截断范围 k 的精度复核（9 个代表形状，15×15 探针，参考解 3000 点，k = 2/3/4 × N = 4/8/16）：各列下 k = 2 误差均最低，k = 4 起明显退化，5.1 的核选型无精度代价 |
| `erf_and_kernel_study.py` | erf 逼近候选的初步对比；闭式与现状离散滤镜在理想硬边下的差异 |
| `wallace_erf_comparison.py` | 辨明 Wallace 所用 erf 即 A&S 7.1.27 去 a₃ 项；复验 7.1.27 公布的误差上限（实测 4.659e-4 ≤ 5e-4）并对比三式传导误差 |
...
| `erf_verification.py` | 复验 A&S 7.1.26 公布的误差上限（实测 1.394e-7 ≤ 1.5e-7）与奇函数性；并评估有界区间多项式替代方案 |
| `anisotropic_reduction_study.py` | 各向异性归约的正确性验证：以原空间暴力二维卷积为真值，确认"按 (1/σx, 1/σy) 缩放 + 椭圆角"无系统偏差（N=32 时各场景 ≤ 0.51 灰度级，接近暴力参考解的网格精度下限） |
| `aspect_vs_corner_study.py` | 分离"归一化长宽比"与"椭圆角偏心度"两个因素（21×21 探针）：长宽比本身不是主因——长宽比 20 配正圆角 N=4 仅 0.90，低于长宽比 2.5 的 2.06；把各向异性 1:3 场景的角改回正圆后 N=4 从 4.22 降到 2.17，故主因是角的偏心度。该结论是两个角半轴的比值 `q` 进入分档判据的依据 |
| `quadrature_ratio_threshold_study.py` | 求积误差随归一化角半轴比值的变化：比值为 1 时 N=4 已达 3.04 灰度级，且误差随比值非单调（1.75 处 6.00 高于 2.0 处 4.83）；同时提供后续判据搜索所用的估计式与探针布点函数 |
| `quadrature_dispatch_search.py` | 分档判据的首轮阈值搜索（272 个归一化形状，7×7 探针）：确认单看角半径占半尺寸的比例或单看半轴比值的各种阈值组合中，哪些能让 N=4 一档不超过统一取 8 的最差值。该轮探针过稀、数值系统性偏低，最终判据以 `quadrature_dispatch_rescan.py` 为准 |
| `quadrature_dispatch_rescan.py` | 分档判据的定稿依据：①固定间距探针下半尺寸对误差无影响（角半轴固定时半尺寸 4→32 误差不变），仅角半轴逼近半尺寸时才变化；②(rx, ry) 两维误差面；③337 个形状（6 组半尺寸 × 角半轴两维各 9 档，41×41 探针，参考解 400 点）上 N=4/8/16/32 的最差值统计与判据对照：`q > 2` 下 N=4 一档最差 6.16、不超过统一取 8 的 7.90，余量 22%（`q > 2.5` / `q > 3` 形式上通过但余量仅 4% / 1%，不可依赖）；④参考解 400 → 800 点在最差形状上漂移 0.010 灰度级的自检 |
| `quadrature_convergence_study.py` | 外层求积收敛阶的受控实验（固定沿 y 积分，只改窗口内被积函数的光滑性，用相邻 N 的结果差测阶）：窗口由核截断时 2.00 阶，由形状截断（端点落在最外行）时降为 1.50 阶，直边区与圆角区交界的二阶导跳变不改变阶数 |
...
测试序列至少覆盖三类：① 单个大 σ 阴影（最能体现解除 `MAX_BLUR_SIGMA` 与省去多 pass 的收益）；② 同参数阴影 × N（验证合批成立）；③ 不同参数阴影 × N（验证合批打断的代价）。每类各跑快路径与强制回落两组对照。另需单独对照求积点数分档的两档（同一形状分别强制 4 与 8），量出那 8 次 `Φ̃` 在帧耗时里的实际占比——分档的收益此前只按调用次数推算，未经实测。

预期与风险：pass 数与中间纹理应显著下降；但每像素成本上升（圆角路径 10 或 18 次 `Φ̃`、每次含一次除法与四次乘加），在**小 σ + 大面积**的场景下有可能反而变慢——因为回落路径的模糊在缩小的 render target 上做，而闭式路径逐像素全分辨率求值。若实测确认该场景存在回退，需补一条"σ 小于某阈值时仍走回落路径"的判定；该阈值只能由实测给出，不预设。
...
| 阶段 | 文件 | 说明 |
|------|------|------|
| 1 | `src/gpu/processors/` + `src/gpu/glsl/processors/`：`RectBlur` / `RRectBlurFragmentProcessor`（各 `.h` + `.cpp` 及 GLSL 对应） | 新增 Blur 系 2 个 FP：`RectBlur`（`shapeBlurRectCoverage`，零采样）、`RRectBlur`（`shapeBlurRRectCoverage`，内层闭式 + 外层 4 / 8 点求积 + 内部实心区跳过，点数由构造参数给出并写入 shader 源码、进 program key）；共享 `shapeBlurErf`（A&S 7.1.27，纯有理式）/ `shapeBlurCDF`（±2σ 截断归一化）/ `shapeBlurRectCoverage` / `shapeBlurSliceCoverage`（椭圆双半轴）函数链，由 `AppendRectBlurFunctions` / `AppendRRectBlurFunctions` 用 `addFunction` 以固定名字发射 |
...
| 6 | `src/gpu/processors/` + `src/gpu/glsl/processors/`：`RectInnerShadow` / `RRectInnerShadowFragmentProcessor`；`src/layers/layerstyles/InnerShadowStyle.cpp` | Inner 系 2 个 FP 与集成：`(1 − coverage) × maskCoverage`；新增 `shapeMaskRectSDF` / `shapeMaskRRectSDF` / `shapeMaskCoverage` 函数（mask 在 content 像素空间求值，角半轴为 `vec2`）；两套坐标由两个 `CoordTransform` 各自的矩阵承担（5.4）；同样限定 spread ≠ 0、`extraSourceType()` 保持现状不变 |
...
5. **复杂圆角矩形（每角独立半径）**：数学上可行——per-corner 下每行横截面仍为区间，左右边界各自由本行角半径解析给出，`shapeBlurSliceCoverage` 由"单一对称半宽"推广为"按 y 区间分段选角 + 双边界"，闭式结构（内层 CDF 差 + 外层固定点数求积）不变；本方案已按椭圆双半轴实现（5.4），推广时每个角各带自己的一对半轴即可，无需再改半宽公式的形式。代价是 shader 分支增多、每行约两倍边界计算、失去象限对称折叠。Figma 的阴影同样只为统一圆角提供闭式单 shader，per-corner 形状回落其通用 tile 路径（3.3）。推广可按 `ComplexNonAARRectGeometryProcessor` 的分区思路进行。
6. **Stroke（描边）阴影的闭式优化**：理论上可用"外圈、内圈各调一次 `shapeBlurRRectCoverage` 取差"实现（卷积线性），但本期不走，原因有三：① **RRect stroke 的同心近似误差**——闭式双圈把内圈视为 inset RRect（同心椭圆弧），而真实描边的内外边界是等距曲线（非椭圆），粗描边、扁椭圆（长短轴比 > 2）、半描边宽度超角半径时，同心近似在法线厚度上与真实描边误差显著（`Canvas.cpp` 的 `UseDrawPath` 正是检测这些失效场景并回落通用 stroker 的现有逻辑）；Rect stroke 无此问题（inset/outset 精确），但使用情况尚不确定。② Figma 未提供 stroke 专用闭式 shader（3.2 要点 4），本期跟随不走。③ 成本收益未明——闭式实现对 stroke 的覆盖受同心近似限制、且描边阴影使用频率低，成本能否被收益覆盖尚未可知。若未来评估值得做，仅需在 `RRectBlurFragmentProcessor` 加 Stroke 模式（外圈、内圈各调一次 `shapeBlurRRectCoverage` 取差）。

7. **提高回落路径的核采样精度（独立改进）**：4 的精度结论显示，快 / 回落两路径的差异主要来自现状滤镜按整数像素偏移点采样源图，σ 小时对核欠采样（σ=1、硬边源时达 51 个灰度级），比快路径在等向典型形状下的近似误差（求积 0.68~2.06 + erf 0.12，均为灰度级）大一个数量级。若后续希望两路径在小 σ 下也逐像素接近，应改进的是**回落路径**——例如按 σ 自适应提高 tap 密度或改用双线性半像素偏移 tap。与本方案独立，收益覆盖所有走通用滤镜的效果（不限阴影）。

8. **erf 换为有界区间拟合多项式**：σ 归一化 + 核截断使 erf 实参恒在 `|z| ≤ k/√2` 内，该区间上 9 阶多项式即可达 3.5e-8，比已采用的 7.1.27（4.7e-4）高四个数量级，且同样无 `exp`、还省去一次除法；利用 erf 的奇函数性写成 `z·P(z²)` 可再省项数。本期不采用，因其系数需自行拟合、无文献出处，且与截断范围 `k` 隐式耦合。它是 5.2 升级序列的第二级——仅在 7.1.26 的 `exp` 开销也不可接受时才考虑；替换只需改 `shapeBlurErf` 一个函数，但必须同时锁定 `k` 并在注释中标注该多项式的有效区间。

9. **统一的 dither 机制（独立改进）**：8-bit 输出下的长距离平缓渐变存在量化 banding，与本方案的近似精度无关（6.1）。若要消除，应在 TGFX 层面引入统一的 dither（Figma 的做法是给每个输出叠加约 `1/255 × ±0.5` 的哈希噪声），覆盖阴影、渐变、模糊等所有产生平缓过渡的路径，而非只给阴影加。需一并解决 baseline 截图带随机噪声后的比较容差问题。
