...
StatisticsModel::StatisticsModel(tracy::Worker& w, ViewData& vd ,View* v ,QObject* parent): QAbstractTableModel(parent),
  view(v),
  viewData(vd),
  worker(w),
  statAccumulationMode(AccumulationMode::SelfOnly),
  statisticsMode(StatMode::Instrumentation),
...
void StatisticsModel::openSource(const char* fileName, int line, const tracy::Worker& worker, const View* view) {
  targetLine = line;
  selectedLine = line;
  targetAddr = 0;
  sourceFiles.clear();
...
void StatisticsModel::parseSource(const char* fileName, const tracy::Worker& worker, const View* view) {
      ...
      for(auto it = slz.begin(); it != slz.end(); ++it) {
        if(it->second.total != 0 && it->second.min <= st) {
          ...
          if(!matchFilter(name, file)){
            ...
            if( cit != statCache.end() &&
                cit ->second.range == view->m_statRange &&
              ...
              if(cit->second.count != 0) {
                ...
                srcloc.push_back_no_space_check(
                  SrcLocZonesSlim{it->first,
                              cit->second.threadNum,
                                cit->second.count, cit->second.total});
              }
            }
            else {
              ...
              for(auto& v : it->second.zones) {
                ...
                if(start >= min && end <= max) {
                  ...
                  else if(statAccumulationMode == AccumulationMode::AllChildren || !isZoneReentry(z)) {
                    total += zt;
                    cnt++;
                    threads.emplace(v.Thread());
                  }
              ...
              RangeSlim{view->m_statRange.min, view->m_statRange.max,view->m_statRange.active}, statAccumulationMode,
              it->second.zones.size(), cnt, total, threadNum};
            }
          }
          else {
            slzcnt++;
        ...
        if(it->second.total != 0) {
          ...
          if (matchFilter(name, file)) {
            srcloc.push_back_no_space_check(
              SrcLocZonesSlim {
                it->first,
                static_cast<uint16_t>(it->second.threadCnt.size()),
                count, total
            });
          }
        }
  ...
  switch (column) {
    case Column::NameColumn :
      if(ascendingOrder) {
        tracy::pdqsort_branchless(srcData.begin(), srcData.end(),
          ...
          return strcmp(worker.GetZoneName(worker.GetSourceLocation(lhs.srcloc)), worker.GetZoneName(worker.GetSourceLocation(rhs.srcloc))) < 0;
        });
      }
      else {
        tracy::pdqsort_branchless(srcData.begin(), srcData.end(),
    ...
    break;
        ...
        case Column::MtpcColumn :
          if(ascendingOrder) {
            tracy::pdqsort_branchless(srcData.begin(), srcData.end(),
              [](const auto& lhs, const auto& rhs){return double (lhs.total) / double(lhs.numZones) < double(rhs.total)  / double(rhs.numZones) ;});
          }
          else {
            tracy::pdqsort_branchless(srcData.begin(), srcData.end(),
              [](const auto& lhs, const auto& rhs){return double(lhs.total) / double(lhs.numZones) > double(rhs.total) / double(rhs.numZones);});
