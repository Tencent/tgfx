...
// 显式实例化模板函数
template std::shared_ptr<tgfx::ShapeLayer> LayerUtils::GetOrCreateLayer<tgfx::ShapeLayer>(tgfx::Layer*, const JsElement&);
template std::shared_ptr<tgfx::TextLayer> LayerUtils::GetOrCreateLayer<tgfx::TextLayer>(tgfx::Layer*, const JsElement&);

void LayerUtils::UpdateCanvasMatrix(tgfx::Layer *layer, const JsRect &canvasRect, const JsRect &viewBox) {
    tgfx::Matrix canvasMatrix = tgfx::Matrix::MakeTrans(canvasRect.x - viewBox.x, canvasRect.y - viewBox.y);
    canvasMatrix.preScale(canvasRect.width / viewBox.width, canvasRect.width / viewBox.width);
    layer->setMatrix(canvasMatrix);
    tgfx::Rect rect = {};
    rect.setXYWH(viewBox.x, viewBox.y, viewBox.width, canvasRect.height);
...
void LayerUtils::UpdateShape(tgfx::Layer *layer, const std::vector<JsElement> &elements) {
    ...
    double averageCircleTime = countUpdateCircle > 0 ? totalUpdateCircleTime / countUpdateCircle : 0.0;
    ...
    // 打印到一行
    if (enableTimeStats) {
        std::cout << "UpdateRect: 平均耗时 = " << averageRectTime << " 微秒, 执行次��� = " << countUpdateRect << ", 耗时占比 = " << rectTimeRatio << "%; "
                  << "UpdateCircle: 平均耗时 = " << averageCircleTime << " 微秒, 执行次数 = " << countUpdateCircle << ", 耗时占比 = " << circleTimeRatio << "%; "
                  << "UpdateText: 平均耗时 = " << averageTextTime << " 微秒, 执行次数 = " << countUpdateText << ", 耗时占比 = " << textTimeRatio << "%; "
                  << std::endl;
    }
}
...
const auto typeface = tgfx::Typeface::MakeFromPath(ProjectPath::Absolute("../../resources/font/NotoSansSC-Regular.otf"));
